...
2. A Discord webhook URL for notifications.
3. A GitHub account and Render account.
...
Create a webhook under **Discord channel → Edit Channel → Integrations →
Webhooks**, then copy its URL into the `DISCORD_WEBHOOK_URL` environment
variable in Render. Do not place the actual webhook URL in GitHub.

The detector sends one **ISH BOOTY DETECTED** message when a new live game is
detected. It does not repeat the message during the same match. After the match
ends, it resets and can notify again for the next match.
